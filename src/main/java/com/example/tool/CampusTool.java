...
public class CampusTool {
    ...
    public String executeCourseAdjustment(String teacherNo, String courseName,
        ...
        try {
            ...
            String major = (String) courses.get(0).get("major");
            ...
            for (String m : allMajors) {
                int cnt = jdbc.queryForObject(
                    ...
                    Integer.class, m, targetDay, targetSlot, courseId);
            ...
            if (roomConflict > 0) return "调课失败：教室" + targetRoom + "在" + targetDay + targetSlot + "已被占用";

            // 验证建筑规则
            List<String> allowed = getAllowedBuildings(major);
            ...
            if (!allowed.contains(targetBuilding))
                return "调课失败：" + major + "专业不能使用" + targetBuilding + "的教室";

            // 执行更新
            jdbc.update("UPDATE course SET schedule_day=?, schedule_time=?, classroom=? WHERE id=?",
                    targetDay, targetSlot, targetRoom, courseId);

            // 发通知
            String notice = String.format("【调课通知】《%s》已从 %s %s 调整至 %s %s @%s。原因：%s。请 %s 专业同学注意。",
            ...
            jdbc.update("INSERT INTO notice (title, content, target_major, publish_time) VALUES (?,?,?,?)",
                    "调课通知", notice, major, LocalDateTime.now());

            log.info("调课成功: {} {} {} -> {} {} {}", cname, originalDay, originalSlot, targetDay, targetSlot, targetRoom);
            return "调课成功！《" + cname + "》已调整至" + targetDay + " " + targetSlot + " @" + targetRoom
                    + "，已通知" + major + "专业学生。";
        } catch (Exception e) {
